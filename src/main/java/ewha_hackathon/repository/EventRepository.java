...
public interface EventRepository extends JpaRepository<Event, Long>, JpaSpecificationExecutor<Event> {
    ...
    @Query(value = "SELECT * FROM event ORDER BY heart_count DESC LIMIT 3", nativeQuery = true)
    ...
    @Query("SELECT h.event FROM Heart h WHERE h.user = :user")
    List<Event> findByUserHeart(@Param("user") User user);

    @Query("SELECT r.event FROM Rsvp r WHERE r.user = :user")
    List<Event> findByUserRsvp(@Param("user") User user);

    @Query("SELECT e FROM Event e WHERE e.user_id = :user")
    List<Event> findByUserRegister(@Param("user") User user);
